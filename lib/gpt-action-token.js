...
export function readGptRefreshToken(request) {
  ...
  if (!match) {
    const details = { authorizationHeaderPresent: Boolean(authorization) };
    console.warn("GPT_ACTION_AUTH_SHAPE", details);
  ...
  if (parts.length !== 4 || parts[0] !== "vg1") {
    const details = safeTokenShape(token);
    console.warn("GPT_ACTION_AUTH_SHAPE", details);
  ...
  try {
    ...
    const details = { ...safeTokenShape(token), structureValid: true };
    console.warn("GPT_ACTION_AUTH_SHAPE", details);
