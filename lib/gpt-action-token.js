...
function normalizeBearerToken(value) {
  ...
  // Remove one or more accidental bearer prefixes.
  while (/^Bearer\s+/i.test(token)) {
    token = token.replace(/^Bearer\s+/i, "").trim();
  }

  // Remove common copy/paste wrappers.
  if (
  ...
  // GPT Actions may preserve extra wrapper text around an API key. Extract only
  // the exact self-contained VAELONS token shape without accepting arbitrary text.
  const embedded = token.match(/vg1\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/);
...
function safeTokenShape(token) {
  return {
    length: token.length,
    startsWithVg1: token.startsWith("vg1."),
...
export function readGptRefreshToken(request) {
  ...
  if (!match) {
    ...
    error.status = 401;
    error.details = { authorizationHeaderPresent: Boolean(authorization) };
    throw error;
  }
  ...
  if (parts.length !== 4 || parts[0] !== "vg1") {
    ...
    error.status = 401;
    error.details = safeTokenShape(token);
    throw error;
  }
  ...
  try {
    ...
    error.status = 401;
    error.details = { ...safeTokenShape(token), structureValid: true };
    throw error;
  }
}
