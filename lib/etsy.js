...
async function resolveVaelonsShop(accessToken) {
  ...
  const shop = await parseResponse(response, "VAELONS mağaza doğrulaması");
  ...
  if (!shop?.shop_id || actual !== expected) {
    const error = new Error(`Yetkilendirilen mağaza VAELONS değil: ${shop?.shop_name || "bilinmiyor"}`);
    error.status = 403;
    throw error;
  }
...
export async function publishDraftListing({ listingId, refreshToken }) {
  ...
  if (String(listing?.shop_id) !== String(shop.shop_id)) {
    const error = new Error("Bu listing VAELONS mağazasına ait değil.");
    error.status = 403;
    throw error;
  }
